...
function charTohex(message) {
    var message_length = message.length;
    var hex_message = "";
    var cc;
    for (i = 0; i < message_length; i++) {
        cc = message.charAt(i);
        hex_message = hex_message.concat(cc.charCodeAt(0).toString(16));
    }
    return hex_message;
}
...
function hello(message, key) {
    ...
    var message_length = message.length;
    if (message_length < 8) {
        /*  var due=8-message_length;
for(i=1;i<=due;i++)
 {
 	message=message.concat(null);
 }
 var hex=charTohex(message);
 console.log(hex);
 alert(main(key_ready,hex));*/
        var hex = charTohex(message);
        var due = (16 - hex.length);
        console.log("Hexa value:" + hex);
        console.log("Length:" + hex.length);
        for (i = 1; i <= due; i++) {
            hex = hex.concat(0);
        }
        // console.log("Final hex before send"+hex);
        //alert(main(key_ready,hex));
        var real_message = main(key_ready, hex);
        ...
        var hex_val = "";
        var part = Math.floor(message_length / 8);
        var f = 0;
        var k = 1;
        while (k <= part) {
            //alert(f);
            var line = message.substring(f, f + 8);

            var hex = charTohex(line);
            hex_val = hex_val.concat(main(key_ready, hex));
        ...
        if (message_length > part * 8) {
            var small = message.substring(part * 8, message_length);
            var small_length = small.length;
            /*var small_due=8-small_length;
            for(m=1;m<=small_due;m++)
            {
            	small=small.concat(null);
            }
            var small_hex=charTohex(small);
            hex_val=hex_val.concat(main(key_ready,small_hex)); */
            var small_hex = charTohex(small);
            var small_due = 16 - small_hex.length;
            for (i = 1; i <= small_due; i++) {
                small_hex = small_hex.concat(0);
            }
            hex_val = hex_val.concat(main(key_ready, small_hex));
        }

        //console.log(hex_val);
        //console.log(hex_val.length);
        var real_message = hex_val;
        console.log(real_message);
    ...
    function main(key_ready, message) {
        ...
        for (m = 1; m <= 16; m++) {
            L[m] = R[m - 1];
            var fun_return = processor(R[m - 1], key_ready[m]);
        ...
        var encrypt_binary = "";
        for (i = 0; i < 64; i++) {
            encrypt_binary = encrypt_binary.concat(final.charAt(ip1[i] - 1));
        }

        //2 document.write("<br>");
        //3 document.write("encrypt_binary"+encrypt_binary);
        //obj["encrypt_binary"]=encrypt_binary;
        var final_hexa = "";
        for (i = 0; i < 64; i = i + 4) {
            //document.write("<br>");
            //document.write(encrypt_binary.substring(i,i+4));
            final_hexa = final_hexa.concat(lookupTable1[encrypt_binary.substring(i, i + 4)]);

        }
        return final_hexa;
    }
    //4 document.write("<br>");
    //5 document.write("Encrypted => "+final_hexa);
...
function start() {
    ...
    */
    ...
    var obj = {};
    hello(obj);
    //alert(obj["final_hexa"]+" "+obj["encrypt_binary"]+" "+obj["final"]);
    //document.getElementById("para3").innerHTML=obj["final_hexa"];
    //document.getElementById("para2").innerHTML=obj["encrypt_binary"];
    //document.getElementById("para1").innerHTML=obj["final"];
    //document.getElementById("heading").innerHTML = "Encryption Successfull";
